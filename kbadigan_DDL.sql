...
CONSTRAINT    P_DOG_BIRTH_MONTH CHECK
                  (DOG_BIRTH_MONTH >=1 AND DOG_BIRTH_MONTH <13),
CONSTRAINT    P_DOG_BIRTH_YEAR CHECK
                  (DOG_BIRTH_YEAR >=1980 AND DOG_BIRTH_YEAR <2031),
CONSTRAINT    P_SEX CHECK
                  (UPPER(SEX) in ('M','F'))
...
CONSTRAINT      P_DOG_BREED_PERCENTAGE_FK2          FOREIGN KEY (BREED_ID)
                   REFERENCES P_DOGBREED(BREED_ID),
CONSTRAINT P_PERCENT CHECK (PERCENT>= 0 AND PERCENT <101)
);
...
CONSTRAINT P_OWNERS_UK UNIQUE (OWNER_FNAME,OWNER_LNAME,
STREETADDRESS1,STREETADDRESS2,ZIP),
CONSTRAINT P_ZIP CHECK (ZIP IN ('18704','18708','18651')),
CONSTRAINT P_EMAIL CHECK (EMAIL LIKE '%_@__%.__%')
); 
...
CONSTRAINT      P_OWNER_DOG_INT_FK1          FOREIGN KEY (DOG_ID)
                   REFERENCES P_DOG(DOG_ID),
CONSTRAINT      P_OWNER_DOG_INT_FK2          FOREIGN KEY (OWNER_ID)
...
CONSTRAINT      P_SERVICE_DOGS_FK1          FOREIGN KEY (DOG_ID)
                   REFERENCES P_DOG(DOG_ID),
CONSTRAINT      P_SERVICE_DOGS_FK2          FOREIGN KEY (OWNER_ID_ASSIGNED)
                   REFERENCES P_OWNER(OWNER_ID)
);
